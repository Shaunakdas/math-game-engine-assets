...
using System.Linq;
using SimpleJSON;
public class DiagQANetworkController : MonoBehaviour {
	...
	public void setQAListJSON(string qaJSONText,QuesAnsList quesAnsList){
		...
		Debug.Log ("Values of diagnostic test id"+N["diagnostic_test_id"].Value + qaJSONText);
		quesAnsList.DisplayId = int.Parse(N ["diagnostic_test_id"].Value);
		quesAnsList.PersonalizationType = int.Parse(N ["personalization_type"].Value);
		int maxLives = 4;
...
//			int correctOp = 0;
			...
			for (int j = 0; j < opCount; j = j + 1) {
				AnswerOption ansOp = new AnswerOption(N ["questions"] [i] ["answers"] [j] ["answer_text"].Value,N ["questions"] [i] ["answers"] [j] ["image"].Value, false,int.Parse( N ["questions"] [i] ["answers"] [j] ["short_choice_answer_id"].Value)) ;
...
//			Debug.Log (N[i]["other_details"]["stream_name"].Value);
	...
	public List<UserEntityResult> getUserWeakEntityList(string resultJSONText){
		...
		if (test > 0) {
			diagnosticTestObject.getPersonalizedTestCount (test);
		}
