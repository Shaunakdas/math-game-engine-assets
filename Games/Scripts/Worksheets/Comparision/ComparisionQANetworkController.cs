...
using System.Linq;
using SimpleJSON;
public class ComparisionQANetworkController : MonoBehaviour {

	public static readonly string domain = "localhost:3000";
	ComparisionQAViewController commonQAViewCtrl;
	...
	IEnumerator getQAListNetworkCall(QuesAnsList quesAnsList) {
		commonQAViewCtrl = (ComparisionQAViewController) gameObject.GetComponent(typeof(ComparisionQAViewController));;
	...
	public void setQAListJSON(string qaJSONText,QuesAnsList quesAnsList){
		...
		for (int i = 0; i <   N ["questions"].Count; i = i + 1) {
			...
			for (int j = 0; j < opCount; j = j + 1) {
				...
				if (answerImg == "null")
					answerImg = "";
				int correctOrder =  int.Parse(N ["questions"] [i] ["answers"] [j] ["correct_order"].Value);
				AnswerOption ansOp = new AnswerOption(N ["questions"] [i] ["answers"] [j] ["answer_text"].Value,answerImg, correctOrder,correctOrder,int.Parse( N ["questions"] [i] ["answers"] [j] ["short_choice_answer_id"].Value)) ;
